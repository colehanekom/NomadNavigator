  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='w-full border-b border-[#66666645]'>
      <div className='w-full flex items-center gap-2 py-4'>
        ...
        <TextInput name='comment' styles='w-full rounded-full py-3' placeholder={replyAt ? `Reply @${replyAt}` : "Comment on this post"} register={register("comment", {
...
const PostCard = ({post, user, deletePost, likePost}) => {
  ...
  return (
    <div className='mb-2 bg-slate-200 p-4 rounded-xl'>
      <div className='flex gap-3 items-center mb-2'>
        <Link to = {"/profile/" + post?.userId?._id}>
        <img src={post?.userId?.profileUrl ?? NoProfile} alt={post?.userId?.userName}
        ...
        <div className='w-full flex justify-between'>
          ...
          <span className='text=ascent-2 text-xs'>
            {moment(post?.createdAt ?? "2023-12-14").fromNow()}
          </span>
        </div>
      </div>
